...
Retain `update-profile-information` in `auth-kit.fortify.features` to enable profile changes. `UpdateUserProfileInformation` implements Fortify's `UpdatesUserProfileInformation` contract and accepts a required name plus a unique email address. If a user implements Laravel's `MustVerifyEmail` contract and changes email, it clears `email_verified_at` and sends a new verification notification. Extend or replace the action if the application has additional profile fields or different validation requirements.
