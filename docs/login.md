...
The default guard is `web`; change it with `AUTH_KIT_GUARD` or pass the intended guard explicitly. Use the same guard for credential validation and session login.