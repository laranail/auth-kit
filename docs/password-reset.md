...
Enable password resets by retaining `reset-passwords` in `auth-kit.fortify.features`. Use `AbstractPasswordResetLinkController` to request a reset link and `AbstractNewPasswordController` to accept a reset token and new password. The application must configure mail delivery so users can receive reset links.
...
Retaining or removing `reset-passwords` in `auth-kit.fortify.features` enables or disables this Fortify capability. Ensure application routes and UI do not expose a flow that has been disabled.